...
class BrandsTableViewCell: UITableViewCell {
    ...
    func setCells(at indexPath: IndexPath, using data: [String]) {
        // ブランド名
        brandName.text = data[indexPath.row] //CodeDef.dataList[indexPath.row]
        // 選択済みかどうか
    ...
    func changeCell(at indexPath: IndexPath, using data: [String]) {
        if !(touchStatus) {
            // falseだった場合（未選択→選択）
            accessoryType = UITableViewCellAccessoryType.checkmark
            let brandName = data[indexPath.row] // CodeDef.dataList[indexPath.row]
